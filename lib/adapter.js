...
adapter.run = async function (cid, config, specs, capabilities) {
    ...
    if (typeof jasmineNodeOpts.expectationResultHandler === 'function') {
        jasmine.Spec.prototype.addExpectationResult = adapter.expectationResultHandler(jasmineNodeOpts, jasmine.Spec.prototype.addExpectationResult)
    }
...
adapter.expectationResultHandler = function (jasmineNodeOpts, origFn) {
    return (passed, data) => {
        try {
